...
class Aluno {
	...
	public function savedMessage($aluno) {
		$content = '';
		try {
			$content .= '<br /><div>O aluno <b>"'.$aluno->getNome().'</b>" foi ADICIONADO com sucesso!</div>'.PHP_EOL;
		} catch (Exception $e) {
	...
	public function listarAlunos($alunos, $turma) {
		...
		if (count($alunos) > 0) {
			...
			foreach ($alunos as $aluno) {
				$content .= '<div><b>Id</b> - '.$aluno['id'].'</div>'.PHP_EOL;
				$content .= '<div><b>Nome</b> - '.$aluno['nome'].'</div>'.PHP_EOL;
				$content .= '<div class="pointyWhenOver abrirAluno" key="'.$aluno['id'].'"><b>(Editar)</b></div>'.PHP_EOL;
				$content .= '<div class="pointyWhenOver apagarAluno" key="'.$aluno['id'].'"><b>(Excluir - X)</b></div>'.PHP_EOL;
				$content .= '<br />'.PHP_EOL;
			}
			$content .= '</div>';
		}
	...
	public function alunoForm($aluno, $turma) {
		...
		$content .= '		Ra: <input type="text" name="ra" id="nome" ra="'.$aluno->getRa().'" /><br />'.PHP_EOL;
